...
app.init = function () {
  ...
  var $sketchControl = d3.select("#sketch-control").append("p").attr("class", "sketch-control");
  ...
  $sketch.attr("width", app.sketch.width + app.sketch.margin.left + app.sketch.margin.right).attr("height", app.sketch.height + app.sketch.margin.top + app.sketch.margin.bottom).append("g").attr("transform", "translate(" + app.sketch.margin.left + "," + app.sketch.margin.top + ")");

  var brush = d3.svg.brush().x(x).y(y)
  // .extent(defaultExtent)
  .on("brush", brushed).on("brushend", brushended);

  $sketch.append("g").attr("class", "brush").call(brush).call(brush.event);

  var point = $sketch.selectAll(".point").data(data.values).enter().append("circle").attr("class", "point").attr("cx", function (d) {
    return x(d[0]);
  }).attr("cy", function (d) {
    return y(d[1]);
  }).attr("r", 4).on("click", function (d, i) {
